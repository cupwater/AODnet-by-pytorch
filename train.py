...
parser.add_argument('--threads', type=int, default=4,
                    help='number of threads for data loader to use, if Your OS is window, please set to 0')
parser.add_argument('--exp', default='pretrain',
                    help='folder to model checkpoints')
parser.add_argument('--printEvery', type=int, default=50,
                    help='number of batches to print average loss ')
parser.add_argument('--batchSize', type=int, default=32,
                    help='training batch size')
parser.add_argument('--epochSize', type=int, default=840,
                    help='number of batches as one epoch (for validating once)')
...
scheduler = torch.optim.lr_scheduler.StepLR(
    optimizer, step_size=53760, gamma=0.5)
...
def train(epoch):
    ...
    for iteration, batch in enumerate(trainDataloader, 0):
        ...
        if iteration % args.printEvery == 0:
            print("===> Epoch[{}]({}/{}): Avg. Loss: {:.4f}".format(epoch,
                  iteration+1, len(trainDataloader), epoch_loss/args.printEvery))
            epoch_loss = 0
...
def checkpoint(epoch):
    model_out_path = "./model_pretrained/AOD_net_epoch_relu_{}.pth".format(
        epoch)
